...
apply_block(#{ block_cache := BlockCache } = State) ->
	...
	case whereis(http_bridge_node) of
		...
		BridgePID ->
			...
			case ar_http_iface_client:get_txs(Peers, Mempool, BShadow) of
				{ok, TXs} ->
					ok = ar_storage:write_tx(TXs),
					gen_server:cast(Worker, apply_block);
				_ ->
					ar:warn([
...
handle_block_from_miner(State, BShadow, MinedTXs, BDS, _POA) ->
	...
	State3 =
		apply_validated_block(State2, B#block{ txs = MinedTXs }, PrevBlocks, BI2, BlockTXPairs2),
