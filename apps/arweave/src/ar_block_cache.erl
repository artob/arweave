...
-export([
	...
	get/2, get_earliest_not_validated_from_longest_chain/1,
	remove/2, prune/2
]).
...
%% If the block already exists in the cache, the cache is returned as is, irregardless of
...
%% is overwritten. Raises previous_block_not_found if the previous block is not in the cache.
			...
			{Map3, Set2} =
				case maps:get(H, Map2, not_found) of
					...
					{_B, on_chain, Children} ->
						{maps:put(H, {B, on_chain, Children}, Map2), Set};
					{_B, _Status, Children} ->
						{maps:put(H, {B, validated, Children}, Map2), Set}
				end,
