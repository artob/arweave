...
stop() ->
	...
	file:delete(filename:join(Config#config.data_dir, "header_sync_record")),
	file:delete(filename:join(Config#config.data_dir, "data_sync_state")),
	rocksdb:destroy(filename:join([Config#config.data_dir, ?ROCKS_DB_DIR, "ar_headers_sync_db"]), []),
